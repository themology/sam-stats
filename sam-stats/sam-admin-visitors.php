...
$entries = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}es ORDER BY id DESC LIMIT $offset, $limit" );
        ...
        <?php if( $entries ) { ?>
            ...
            <tr<?php echo $class; ?>>
                ...
                <td><?php
				 ...
				 echo $entry->sam_date."<br />";
				 echo $entry->sam_time;
				 
				  ?></td>
                <td><?php
				
					$time1 = strtotime($entry->sam_time);
					$time2 = strtotime($entry->sam_endtime);
					$diff = $time2 - $time1;
...
$total = $wpdb->get_var( "SELECT COUNT(`id`) FROM {$wpdb->prefix}es" );
